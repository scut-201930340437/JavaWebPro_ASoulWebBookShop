<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %>
<%@ page contentType="text/html;charset=UTF-8" language="java" %>
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
<title>编辑书籍信息</title>

	<%-- 静态包含 base标签、css样式、jQuery文件 --%>
	<%@ include file="/pages/common/head.jsp"%>
	<style type="text/css">
	h1 {
		text-align: center;
		margin-top: 200px;
	}
	
	h1 a {
		color:red;
	}
	
	input {
		text-align: center;
	}
</style>
</head>
<body>

		<div id="header">
			<img class="logo_img" alt="" src="../../static/img/logo.jpg" >
            <c:if test="${not empty requestScope.operation}">
                <span class="wel_word">更新书籍信息</span>
            </c:if>
            <c:if test="${empty requestScope.operation}">
                <span class="wel_word">添加书籍</span>
            </c:if>

			<%-- 静态包含 manager管理模块的菜单  --%>
			<%@include file="/pages/common/manager_menu.jsp"%>
		</div>
		
		<div id="main">
			<form action="/managerBook" method="post">
				<input type="hidden" name="pageNo" value="${param.pageNo}">
				<input type="hidden" name="method" value="${ empty requestScope.operation ? "AddBook" : "UpdateBook" }" />
				<input type="hidden" name="id" value="${ requestScope.bookByID.ID }" />
				<table>
					<tr>
						<td>名称</td>
						<td>作者</td>
						<td>价格</td>
						<c:if test="${not empty requestScope.operation}">
							<td>销量</td>
						</c:if>
						<td>库存</td>
						<td colspan="2">操作</td>
					</tr>		
					<tr>
						<td><input name="name" type="text" value="${requestScope.bookByID.name}"/></td>
						<td><input name="author" type="text" value="${requestScope.bookByID.author}"/></td>
						<td><input name="price" type="text" value="${requestScope.bookByID.price}"/></td>
						<c:if test="${not empty requestScope.operation}">
							<td><input name="sales" type="text" value="${requestScope.bookByID.sales}"/></td>
						</c:if>
						<td><input name="stock" type="text" value="${requestScope.bookByID.stock}"/></td>
						<td><input type="submit" value="提交"/></td>
					</tr>	
				</table>
			</form>
		</div>

		<%--静态包含页脚内容--%>
		<%@include file="/pages/common/footer.jsp"%>

</body>
</html>